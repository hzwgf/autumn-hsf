 ...
 * @version V1.0
 */
public class EndlessLoopUtils {
    static Logger logger = LoggerFactory.getLogger(EndlessLoopUtils.class);
    ...
    static boolean fixEndlessLoop(Object obj, HashSet<Object> chain) {
        List<Field> fields = ReflectionUtils.getFields(obj.getClass());
        ...
        for (Field fd : fields) {
            ...
            if (temp != null) {
                ...
                if (!chain.contains(temp)) {
                    ...
                    try {
                        fd.set(obj, ReflectionUtils.getDefaultValue(clazz));
                    }
                    catch (Exception e) {
                    }
                    return true;
